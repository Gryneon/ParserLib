...
public sealed class DictionaryOperation (RxSCollection regex, string replacewith) : Operation
{
  ...
  internal string Operate (string text, TextDataFormat format)
  {
    foreach (RxS single in OpRegex)
    {
      //Prevent lockups.
      ...
      {
        text = Regex.Replace(text, single, Replacement, format.RegexOptions);
        Debug_ReplacementCount++;
        loop_breaker++;
      }
      if (loop_breaker == 20)
        Debug.Log("ReplaceOperation.Operate", "loop_breaker == 20 evaluates to true.");
    }
    Debug.Log("ReplaceOperation.Operate", $"Debug_ReplacementCount = {Debug_ReplacementCount}.");
    return text;
  }
