...
public sealed class TokenFactory<T> (IEnumerable<TokenRule<T>> rules) where T : struct
{
...
public sealed class TokenFactory (IEnumerable<TokenRule> rules)
{
  internal const string Area = "TokenFactory<T>";
  internal string Input = SE;
  internal SectionCollection CannotMatch = [];
  ...
  public string GetRuleRegex (TokenRule rule, int? index = null)
  {
    string regex = rule?.RuleStringData ?? SE;
