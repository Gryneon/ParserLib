...
""");
  public static TokenNodeGroup GetTokenNodes (string token_type_string, out string? import_group)
  {
    ...
    Log("TokenNodeFactory.GetTokenNodes", "Beginning iteration through nodes");
    foreach (MatchDataSet md in mdc)
    {
      ...
      if (filling_group is null)
        continue;
      Log("TokenNodeFactory.GetTokenNodes", $"Node Processed: {item.Type}");
      ...
      switch (item.Type)
      {
        ...
        case TokenNodeType.Group:
          Log(Area, "Group Encountered. This should never occur.");
